...
% helDiff(lambda gamma, lambda target proton, lambda total).
helDiff(-1,-1/2,-1/2).
helDiff(-1,1/2,-3/2).
helDiff(1,'-1/2',3/2).
helDiff(1,1/2,1/2).
...
f( 1/2, 1/2, 1,1,_,_,1.0).
f(-1/2, 1/2,-1,1,Eta_alpha,Ja,X) :- R is -1.0**(Ja - 1/2),X is Eta_alpha * R.
f(-1/2,-1/2, 1,1,_,_,1.0).
f( 1/2,-1/2,-1,1,Eta_alpha,Ja,X) :-  R is -1.0**(Ja - 1/2),X is Eta_alpha * R.
...
buildSBase :-
  lam_i(Li),lam_g(Lg),lam_f(Lf),helDiff(Lg,Li,La),
  sstate(Ja,La,Lf,Pa,P1,P2,S1,S2,R,Alpha), 
  f(Li,Lip,Lg,Lgp,Pa,Ja,Eta),
  parName(Alpha,Lip,Lgp,Ja,Pa,Parname),
  assertz(salpha(Parname,R,Li,Lg,Lf,Ja,La,Pa,Eta)),
...
parName(Alpha,Li,Lg,Ja,Pa,Name) :- 
  name('V_',Nv),name(Alpha,Na),name('_',Nu),gname(Li,Lii),
  name(Lii,Ni),name(Lg,Ng),Jaa is 2 * Ja,name(Jaa,Nj),name(Pa,Np),
  conc(Nv,Na,X1),
  conc(X1,Nj,X2),
  conc(X2,Nu,X3),
  conc(X3,Np,X4),
...
printSBase :- 
  salpha(Name,Amps,Li,Lg,Lf,Ja,La,Pa,Eta),
  write(Li),write('\t'),
  write(Lg),write('\t'),
  write(Ja),write('\t'),
  write(La),write('\t'),
...
writeSSum(Li,Lg,Lf) :- 
  sm(Li,Lg,Lf,N),
  write(N),write(' = '),
  salpha(Name,Amp,Li,Lg,Lf,_,_,_,_),
  write(' + '),
...
sm(Li,Lg,Lf,N) :- 
  name('S_',Ns),name('_',Nu),gname(Li,Lii),name(Lii,Ni),name(Lg,Ng),gname(Lf,Lff),name(Lff,Nf),
  conc(Ns,Ni,X1),
  conc(X1,Nu,X2),
  conc(X2,Ng,X3),
  conc(X3,Nu,X4),
